...
class Over_Time extends CI_Model 
{
    ...
    public function process($scancode, $image) 
    {
		...
		if($numrow > 0) {
			...
			$sql = "SELECT * FROM manual_log WHERE employeecode = ? AND logdate = ? AND timetype = 'OT' ORDER BY logid DESC";
			...
			if($numrow > 0) {
				...
				if(empty($login)) {
					...
					//$html .= '</div></li>';
					$html .= '<div class="media-heading-small"> Over Time Log IN '.$currenttime.'</div></div></li>';

					$params = array('calendartype' => $calendartype, 'logdate' => $dtrdate, 'timetype' => 'OT');
					...
					$html .= '<div class="media-heading-small"> Over Time Log IN '.$login.'</div>';
					$html .= '<div class="media-heading-small"> Over Time Log OUT '.$currenttime.'</div>';
					$html .= '</div></li>';
